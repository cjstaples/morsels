import axxpairs.combo
import axxpairs.combo.storage
from axxpairs.combo.combinatorics import xuniqueCombinations
...
class MyPairs:
    ...
    def __init__( self, options, filter_func = lambda x: True, previously_tested = [[]], n = 2 ):
        ...
        self.__n = n
        self.__pairs = axxpairs.combo.storage.Pairs(n)
    ...
    def resort_working_array( self, chosen_values_arr, num ):
        for item in self.__working_arr[num]:
            ...
            for i in range(0, self.__n):
                ...
                new_combs.append(set([axxpairs.combo.storage.key(z) for z in xuniqueCombinations(chosen_values_arr + [item], i + 1)]) - self.__pairs.get_combs()[i])
            # weighting the node
...
__export__ = [MyPairs, get_max_comb_number]
